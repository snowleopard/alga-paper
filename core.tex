...
then abstract from this representation by focusing on the laws that these
primitives satisfy (see~\S\ref{sec-algebra}).
...
graphs. As we will see in~\S\ref{sec-algebra}, the overlay and connect operations
are very similar to addition and multiplication, therefore connect has a higher
...
overlaying its vertices and connected pairs of vertices.
...
\noindent
For example, \hs{clique@\,@[1,2,3]} is graph $1 \rightarrow 2 \rightarrow 3$, i.e.
