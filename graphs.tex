...
By embedding the core graph construction primitives into an algebraic data type \hs{Expr}
we obtain the following \hs{Graph} instance (it is tempting to name the data type
\hs{Graph} instead of \hs{Expr}, but that will likely cause confusion):
...
\subsection{Adjacency map, depth-first search, and topological sort}\label{sub-adjacency-map}
...
\hs{graphFromEdges} provided by \textsf{containers} that requires an adjacency
list as the input. We can obtain the adjacency list from \hs{AdjacencyMap} as
follows:
...
A similar \hs{newtype Dfs} wrapper can be created for the depth-first search
algorithm. It will also trivially satisfy the \hs{Graph} axioms, but will have
a different equivalence relation, since \hs{dfs (1 + 2)} $\neq$ \hs{dfs (1 * 2)}.
...
One can implement the reflexive and irreflexive \hs{Graph} instances very similarly to
the \hs{Symmetric} implementation presented in~\S\ref{sub-undirected}, by wrapping the
\hs{Relation} into a \hs{newtype} and giving it a custom \hs{Eq} instance based on the
\hs{reflexiveClosure} or \hs{irreflexiveClosure}, respectively.

We can define
\hs{class Graph@\,\,\blk{g}@ => ReflexiveGraph@\,\,\blk{g}@} and
\hs{class Graph@\,\,\blk{g}@ => IrreflexiveGraph@\,\,\blk{g}@}
...
We can implement a \hs{Graph} instance for transitive graphs by wrapping
...
A subclass \hs{class Graph@\,\,\blk{g}@ => TransitiveGraph@\,\,\blk{g}@} can be defined to distinguish
